...
import type { MultimediaChoiceContent, Option, OptionMedia } from "../../../types/H5P/content/multimedia-choice.ts";
...
export class MultiMediaChoiceGenerator implements ContentGenerator {
  ...
  //@todo types
    generate(base: TranslationPair[], template: MultimediaChoiceContent): MultimediaChoiceContent[] {
     ...
     base.map((tp, i) => {
      ...
      //  newTemplate.media.type = this.generateMedia(tp, newTemplate.media.type, "source");
       newTemplate.options = this.generateOptions(tp, i, base, newTemplate.options);
     });
     return [template];
  }
  generateMedia(tp: TranslationPair, template: OptionMedia, type: "source"|"translation"): OptionMedia{
