...
export class PairsWordExpander {
  ...
  _langSpecificWordModifier: (
    words: string[],
    needsModified: boolean[],
  ) => void;
  constructor(
    dictionary: Dictionary,
    langSpecificWordModifier: (
      words: string[],
      needsModified: boolean[],
    ) => void,
  ) {
  ...
  expand(data: TranslationPair[]) {
    ...
    data.forEach((tp, i, data) => {
      data.forEach((tpToTest, j) => {
        if (
          tpToTest.translation === tp.translation &&
  ...
  expandTranslationPair(tp: TranslationPair) {
    ...
    const expandWord = (word: string) => {
      ...
      const hasTranslations = recordDetails.find((de) => de[0].length > 0);
      //.hasTranslations());
      if (hasTranslations) {
        expanded = this.addTranslationPair(
          hasTranslations[0],
          word,
          translationPairs,
        );
        ...
        for (let de of des) {
          ...
          if (hasFormOf) {
            ...
            forms.forEach((form) => {
              ...
              const dictionaryEntries = this.getTranslatedWord(form);
              const hasTranslations = dictionaryEntries.find(
                (de) => de[0].length > 0,
              );
              if (hasTranslations) {
                expanded = this.addTranslationPair(
    ...
    expandedWords.forEach((expanded, i) => {
      ...
      const spanishCliticEndings = [
        ...
        "lesla",
        "leslos",
        "leslas", // rare but grammatically possible
      ];
      if (!expanded) {
        const needToModify = words[i];
        if (needToModify) {
          const modifiedReflexiveStripped = removeEnding(
            needToModify,
            spanishCliticEndings,
          );
    ...
    expandedWords.forEach((expanded, i) => {
      if (!expanded) {
        const word = words[i];
        if (word) this._untranslatedWords.add(word);
      }
    });
  ...
  addTranslationPair(
    ...
    let trans: string | undefined = undefined;
    if (dts.length) {
      trans = dts.map((x) => x.word).join(", ");
    }
